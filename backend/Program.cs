...
string? seniorConnString = builder.Configuration.GetConnectionString("SeniorConnection");
string? localInitConnString = builder.Configuration.GetConnectionString("LocalInitConnection");

if(connStr == null || seniorConnString == null || localInitConnString == null) { 
    throw new Exception("Missing required connection strings in appsettings.json"); 
}

using (var initConn = new SqlConnection(localInitConnString))
{
    initConn.Open();
...
// ============================================================
var connectionsFile = Path.Combine(AppContext.BaseDirectory, "connections.json");

// GET /api/connections - returns saved configs (passwords hidden)
app.MapGet("/api/connections", () =>
{
    ...
    foreach (var prop in doc.RootElement.EnumerateObject())
    {
        ...
        {
            if (p.Name.ToLower().Contains("password") || p.Name.ToLower().Contains("senha"))
                inner[p.Name] = null; // hide
            else
...
// POST /api/connections/{id}/test - tests without saving
app.MapPost("/api/connections/{id}/test", async (string id, ConnectionConfig cfg) =>
{
    if (string.IsNullOrWhiteSpace(cfg.Server) || string.IsNullOrWhiteSpace(cfg.Database))
        return Results.BadRequest(new { success = false, error = "Servidor e Database são obrigatórios." });

    var cs = $"Server={cfg.Server},{cfg.Port};Database={cfg.Database};User Id={cfg.Username};Password={cfg.Password};TrustServerCertificate=true;Connect Timeout=8;";
...
// POST /api/connections/{id} - saves config to JSON file
app.MapPost("/api/connections/{id}", async (string id, ConnectionConfig cfg) =>
{
    Dictionary<string, object> all = new();

    if (File.Exists(connectionsFile))
    {
        var existing = await File.ReadAllTextAsync(connectionsFile);
        all = System.Text.Json.JsonSerializer.Deserialize<Dictionary<string, object>>(existing)
              ?? new();
    }

    all[id] = cfg;
    var opts = new System.Text.Json.JsonSerializerOptions { WriteIndented = true };
    await File.WriteAllTextAsync(connectionsFile, System.Text.Json.JsonSerializer.Serialize(all, opts));

    return Results.Ok(new { message = "Salvo com sucesso." });
});
